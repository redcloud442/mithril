...
export const depositRequestSchema = z.object({
  amount: z
    ...
    .refine(
      (amount) =>
        parseInt(amount, 10) >= 500 && parseInt(amount, 10) <= 1000000,
      {
        message:
          "Amount must be at least 500 pesos and less than 1,000,000 pesos",
      }
    ),
...
export const registerUserCodeSchema = z.object({
  code: z.coerce
    ...
    .min(6)
    .max(10)
    .regex(/^[A-Za-z0-9]{6,10}$/, "Code must be 6–10 letters or numbers")
    .trim(),
});
...
export const PromoPackageSchema = (
  ...
  return z.object({
    amount: z
      .string()
      .trim()
      .min(3, "Minimum amount is 5000 pesos")
      .refine((val) => Number(val) >= 5000, {
        message: "Minimum amount is 5000 pesos",
      })
